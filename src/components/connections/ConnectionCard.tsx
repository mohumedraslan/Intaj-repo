...
export default function ConnectionCard({ name, type, status, stats, color, icon }: ConnectionCardProps) {
  const isConnected = status === "Connected";
  const statusColor = isConnected ? "green" : "yellow";
  
  return (
    <div className={`bg-[#141517]/80 backdrop-blur-md p-6 rounded-2xl border border-blue-500/10 hover:border-${color}-500/50 transition-all duration-300`}>
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center space-x-3">
          <div className={`w-12 h-12 bg-${color}-500 rounded-xl flex items-center justify-center overflow-hidden`}>
