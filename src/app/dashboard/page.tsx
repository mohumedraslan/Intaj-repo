            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="relative">
                <input
                  type="text"
                  placeholder="Search..."
                  className="bg-[#1f2024] border border-gray-600 rounded-lg px-4 py-2 pl-10 text-gray-200 focus:border-blue-500 focus:outline-none w-64"
                />
                <svg
              ...
              >
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path
                    ...
                    strokeLinejoin="round"
                    strokeWidth="2"
                    d="M15 17h5l-5.405-5.405a2.032 2.032 0 01-.595-1.43V11a6.5 6.5 0 11-13 0v-0.834c0-.532-.217-1.04-.595-1.43L0 17h5m10 0v1a3 3 0 11-6 0v-1m6 0H9"
                  />
                </svg>
                <div className="absolute -top-1 -right-1 w-3 h-3 bg-red-500 rounded-full animate-pulse"></div>
              </button>
          ...
          {loading ? (
            ...
            <div className="flex items-center justify-center h-full">
              <div className="text-red-500">
                Error loading dashboard data: {error.message || 'An unknown error occurred'}
              </div>
            </div>
          ) : (
            <div data-tour="dashboard-overview">
              ...
              >
                ...
                <div className="bg-[#1f2024] glass-card p-6 rounded-2xl hover:border-blue-500/30 transition-colors">
                  ...
                  {stats.weeklyBotGrowth > 0 && (
                    <div className="text-xs text-green-500 mt-2">
                      +{stats.weeklyBotGrowth} this week
                    </div>
                  )}
                </div>
                {/* Today's Conversations */}
                <div className="bg-[#1f2024] glass-card p-6 rounded-2xl hover:border-purple-500/30 transition-colors">
                  ...
                  {stats.conversationGrowth !== 0 && (
                    <div
                      className={`text-xs ${stats.conversationGrowth > 0 ? 'text-green-500' : 'text-red-500'} mt-2`}
                    >
                      {stats.conversationGrowth > 0 ? '+' : ''}
                      {stats.conversationGrowth}% vs yesterday
                    </div>
                  )}
                </div>
                {/* Response Time */}
                <div className="bg-[#1f2024] glass-card p-6 rounded-2xl hover:border-cyan-500/30 transition-colors">
                  <div className="flex items-center justify-between mb-4">
                    <div className="w-12 h-12 bg-cyan-500/20 rounded-xl flex items-center justify-center">
                      <svg
                        className="w-6 h-6 text-cyan-500 animate-pulse"
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                      >
                  ...
                  {stats.responseTimeImprovement !== 0 && (
                    <div
                      className={`text-xs ${stats.responseTimeImprovement < 0 ? 'text-green-500' : 'text-red-500'} mt-2`}
                    >
                      {stats.responseTimeImprovement < 0 ? '' : '+'}
                      {stats.responseTimeImprovement.toFixed(1)}s vs avg
                    </div>
                ...
                <div className="bg-[#1f2024] glass-card p-6 rounded-2xl hover:border-green-500/30 transition-colors">
                  ...
                  {stats.satisfactionGrowth !== 0 && (
                    <div
                      className={`text-xs ${stats.satisfactionGrowth > 0 ? 'text-green-500' : 'text-red-500'} mt-2`}
                    >
                      {stats.satisfactionGrowth > 0 ? '+' : ''}
                      {stats.satisfactionGrowth.toFixed(1)}% this month
                    </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
                ...
                <div className="bg-[#1f2024] glass-card p-6 rounded-2xl" data-tour="quick-actions">
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-6">
                    <Link href="/dashboard/chatbots/create">
                      <button
                        className="w-full bg-gradient-to-r from-blue-500 to-purple-500 p-4 rounded-lg text-white font-medium hover:opacity-90 transition-opacity flex flex-col items-center space-y-2"
                        data-tour="create-chatbot-btn"
                      >
                        <svg
                          className="w-6 h-6"
                          fill="none"
                ...
                {/* Recent Activity */}
                <div
                  className="bg-[#1f2024] glass-card p-6 rounded-2xl"
                  data-tour="chatbots-section"
                >
                  <h3 className="text-xl font-bold text-white mb-6 flex items-center space-x-2">
                    <svg
                  ...
                  <div className="space-y-4">
                    {recentActivity?.map(activity => (
                      <div
                        key={activity.id}
                        className="flex items-start space-x-4 p-3 bg-[#23242a] rounded-lg"
                      >
                        <div
    ...
    async function fetchUserProfile() {
      try {
        ...
        if (user) {
          ...
          // Fetch user profile to get onboarding steps
        ...
        <main className="flex-1 overflow-y-auto bg-[#0a0a0b] p-8">
          {loading ? (
            ...
            <div data-tour="dashboard-overview">
              ...
              {!loadingProfile && userId && onboardingSteps && 
                !onboardingSteps.has_dismissed && 
                (!onboardingSteps.created_first_chatbot || 
                 !onboardingSteps.added_data_source || 
                 !onboardingSteps.connected_channel) && (
                <OnboardingChecklist 
                  userId={userId} 
