...
export default function DashboardPage() {
  ...
  const { stats, recentActivity, platformCounts, loading, error, isRealTimeConnected } = useDashboardData();
      ...
      <aside className="w-80 bg-[#141517] border-r border-gray-700/50 flex flex-col">
        ...
        <nav className="flex-1 p-4 space-y-2">
          ...
          <Link 
            ...
            className={`sidebar-link flex items-center space-x-3 p-3 rounded-lg ${
              pathname.includes('/templates') ? 'bg-primary-500/20 text-primary-500' : 'text-gray-400 hover:text-gray-200'
            }`}
          >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
