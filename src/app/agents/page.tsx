...
export default function AgentsPage() {
  ...
  useEffect(() => {
    const fetchUserAgents = async () => {
      try {
        ...
        if (!user) {
          setErrorMessage('Please log in to view your agents');
          setIsLoading(false);
          return;
        }
        ...
        if (chatbotsError) {
          setErrorMessage(chatbotsError.message);
          setIsLoading(false);
          return;
        }
        ...
        const transformedAgents: Agent[] =
          chatbots?.map(chatbot => {
            ...
            const agentType: Exclude<AgentType, 'all'> = chatbot.name
              .toLowerCase()
              .includes('sales')
              ? 'sales'
              : chatbot.name.toLowerCase().includes('marketing')
                ? 'marketing'
                : 'chatbot';
            ...
            return {
              ...
              role:
                chatbot.description ||
                `${agentType === 'chatbot' ? 'Customer Support' : agentType === 'sales' ? 'Sales' : 'Marketing'} Agent`,
        ...
        const avgResolution =
          transformedAgents.length > 0
            ? Math.round(transformedAgents.reduce(acc => acc + 94, 0) / transformedAgents.length)
            : 0;
        ...
        setErrorMessage('Failed to fetch agents data');
      } finally {
        setIsLoading(false);
      }
    };
    ...
    return (
      <div className="min-h-screen neural-grid pt-24 pb-12">
        <div className="flex justify-center items-center">
          <div className="text-xl text-gray-300">Loading agents...</div>
        </div>
      </div>
    );
  }
  ...
  if (errorMessage) {
    return (
      <div className="min-h-screen neural-grid pt-24 pb-12">
        <div className="flex justify-center items-center">
          <div className="text-xl text-red-400">{errorMessage}</div>
        </div>
      </div>
    );
  }
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-xl text-gray-300">
                Manage your intelligent workforce of AI agents
              </p>
            </div>
              ...
              <div key={index} className="glass-card p-6 rounded-xl text-center">
                ...
                {'status' in stat ? (
                  <div className="flex items-center justify-center mt-2">
                    <div
                      className={`w-2 h-2 agent-status-${stat.status?.type || 'unknown'} rounded-full animate-pulse mr-2`}
                    ></div>
                    <span className="text-xs text-green-400">{stat.status?.text || 'Unknown'}</span>
                  </div>
                ) : 'trend' in stat ? (
            ...
            >
              <div className="flex items-start justify-between mb-6">
                <div className="flex items-center space-x-4">
                  <div
                    className={`w-12 h-12 ${agent.type === 'sales' ? 'sales-avatar' : agent.type === 'marketing' ? 'marketing-avatar' : 'agent-avatar'} rounded-full flex items-center justify-center animate-pulse-ai`}
                  >
                    {agent.avatar}
                  </div>
                  <div>
                ...
                <div className="flex items-center space-x-2">
                  <div
                    className={`w-3 h-3 agent-status-${agent.status} rounded-full ${agent.status !== 'offline' ? 'animate-pulse' : ''}`}
                  ></div>
                  <span
                    className={`text-xs ${
                      agent.status === 'online'
                    ...
                    className={`w-6 h-6 ${
                      agent.type === 'sales'
                        ...
                        : agent.type === 'marketing'
                          ? 'bg-green-500'
                          : 'bg-blue-500'
                    } rounded-full ${agent.status !== 'offline' ? 'animate-bot-thinking' : ''}`}
                  ></div>
