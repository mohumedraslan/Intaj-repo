...
export class InstagramIntegration {
  ...
  async sendMessage(
    recipientId: string,
    message: string,
    messageType: 'text' | 'image' | 'story_reply' = 'text'
  ...
  async processWebhookData(
    ...
    try {
      ...
      const response = await fetch(
        `${this.baseUrl}/${mediaId}/comments?fields=id,text,username,timestamp&access_token=${this.config.accessToken}`
      );
      const data = await response.json();
...
export class InstagramConnectionManager {
   ...
   */
  async storeStoryMention(
    chatbotId: string,
    mention: InstagramStoryMention
  ): Promise<{ success: boolean; error?: string }> {
    try {
      const { error } = await this.supabase.from('messages').insert({
        chatbot_id: chatbotId,
